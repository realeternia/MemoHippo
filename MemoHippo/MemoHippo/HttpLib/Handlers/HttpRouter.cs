...
namespace HttpLib.Handlers
{
    public class HttpRouter : IHttpRequestHandler
    {
        ...
        public IHttpRequestHandler DefaultHandler { get; set; }
        ...
        {
            var c = new List<string>(_handlers.Keys);
            c.Sort();
            return c;
        }

        public Task Handle(HttpContext context, Func<Task> nextHandler)
        {
            string function = string.Empty;

            if (context.Request.RequestParameters.Length > 0)
            {
                if (context.Request.RequestParameters.Length == 1)
                {
                    function = context.Request.RequestParameters[0];
                }
                else
                {
                    function = string.Join("/", context.Request.RequestParameters);
                }
            }

            if (DefaultHandler != null && context.Request.Uri.ToString() != "/hello")
            {//用来支持路由模式
                return DefaultHandler.Handle(context, nextHandler);
            }
            else
            {
